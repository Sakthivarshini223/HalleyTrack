...
const Register = () => {
  ...
  const handleRegister = async (e) => {
    ...
    if (!validatePassword(password)) {
      setError("PROTOCOL: MIN 8 CHARS, 1 UPPERCASE, 1 SPECIAL");
      return;
    }
    ...
    try {
      ...
      setError(
        err.response?.data?.message || "REGISTRATION FAILED: SYSTEM CONFLICT"
      );
    }
  };
  ...
  return (
    <div className="min-h-screen bg-black flex items-center justify-center p-4 font-sans text-zinc-200 relative overflow-hidden">
      <div className="absolute inset-0 z-0 overflow-hidden pointer-events-none">
        <div className="absolute top-[-10%] right-[-10%] w-[500px] h-[500px] bg-orange-600/10 blur-[120px] rounded-full animate-pulse" />
        <div className="absolute bottom-[-10%] left-[-10%] w-[400px] h-[400px] bg-zinc-800/20 blur-[100px] rounded-full" />
        <div
      ...
      <div
        className={`w-full max-w-[420px] relative z-10 group ${
          error ? "animate-shake" : ""
        }`}
      >
        <div
        ...
        <div className="relative bg-[#080808] border border-white/5 p-8 md:p-10 rounded-[2rem] backdrop-blur-3xl shadow-2xl">
          <div className="text-center mb-8">
            ...
            <h1 className="text-3xl font-black tracking-tighter text-white uppercase italic">
              NEW <span className="text-orange-600 not-italic">IDENTITY</span>
            </h1>
            <p className="text-[8px] text-zinc-600 font-black uppercase tracking-[0.4em] mt-2">
              HALLYTRACK REGISTRATION
            </p>
          </div>
          ...
          <form onSubmit={handleRegister} className="space-y-4">
            ...
            {error && (
              <div className="flex items-center gap-2 px-4 py-2 bg-red-600/5 border border-red-600/20 rounded-lg animate-in fade-in slide-in-from-top-2 duration-300">
          ...
          <div className="mt-10 flex items-center justify-between border-t border-white/5 pt-6">
            <Link
              to="/"
              className="text-[9px] text-zinc-700 uppercase font-black tracking-widest hover:text-orange-600 transition-colors"
            >
              Return to Login
            </Link>
